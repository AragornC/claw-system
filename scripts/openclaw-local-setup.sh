...
THUNDERCLAW_TELEGRAM_ALLOWED_CHAT_IDS=""
THUNDERCLAW_TELEGRAM_AUTO_REPLY="1"
THUNDERCLAW_TELEGRAM_PUSH_TRADES="1"
THUNDERCLAW_TELEGRAM_PUSH_CHAT_IDS=""
...
ENABLE_TELEGRAM_RELAY="$(prompt_default "是否启用 Telegram->看板 消息同步? (y/n)" "y")"
if is_yes "${ENABLE_TELEGRAM_RELAY}"; then
  THUNDERCLAW_TELEGRAM_BOT_TOKEN="$(prompt_default "Telegram Bot Token（留空=禁用）" "${THUNDERCLAW_TELEGRAM_BOT_TOKEN:-${TELEGRAM_BOT_TOKEN:-}}")"
  if [[ -n "${THUNDERCLAW_TELEGRAM_BOT_TOKEN}" ]]; then
    THUNDERCLAW_TELEGRAM_ALLOWED_CHAT_IDS="$(prompt_default "允许的 chat id（逗号分隔，留空=全部）" "")"
    AUTO_REPLY_TELEGRAM="$(prompt_default "Telegram 来信后是否自动回复? (y/n)" "y")"
    if is_yes "${AUTO_REPLY_TELEGRAM}"; then
      THUNDERCLAW_TELEGRAM_AUTO_REPLY="1"
    else
      THUNDERCLAW_TELEGRAM_AUTO_REPLY="0"
    fi
    PUSH_TRADE_EVENTS="$(prompt_default "是否开启交易事件主动推送到 Telegram? (y/n)" "y")"
    if is_yes "${PUSH_TRADE_EVENTS}"; then
      THUNDERCLAW_TELEGRAM_PUSH_TRADES="1"
      THUNDERCLAW_TELEGRAM_PUSH_CHAT_IDS="$(prompt_default "交易事件推送 chat id（逗号分隔，留空=自动）" "${THUNDERCLAW_TELEGRAM_ALLOWED_CHAT_IDS}")"
      THUNDERCLAW_TELEGRAM_PUSH_EVENTS="$(prompt_default "推送事件类型(open,close,risk)" "open,close,risk")"
    else
      THUNDERCLAW_TELEGRAM_PUSH_TRADES="0"
    fi
  fi
fi
...
THUNDERCLAW_TELEGRAM_ALLOWED_CHAT_IDS=${THUNDERCLAW_TELEGRAM_ALLOWED_CHAT_IDS}
THUNDERCLAW_TELEGRAM_AUTO_REPLY=${THUNDERCLAW_TELEGRAM_AUTO_REPLY}
THUNDERCLAW_TELEGRAM_PUSH_TRADES=${THUNDERCLAW_TELEGRAM_PUSH_TRADES}
THUNDERCLAW_TELEGRAM_PUSH_CHAT_IDS=${THUNDERCLAW_TELEGRAM_PUSH_CHAT_IDS}
THUNDERCLAW_TELEGRAM_PUSH_EVENTS=${THUNDERCLAW_TELEGRAM_PUSH_EVENTS}
PORT=8765
EOF
