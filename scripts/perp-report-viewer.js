  ...
  <script>
    ...
    function init(RECORDS, OHLCV_BY_TF, MARKERS_BY_TF, symbol, ordersPayload) {
      ...
      function orderRelationHtml(order) {
        ...
        if (close) {
          return '<span class="badge ' + (Number.isFinite(pnl) ? (pnl >= 0 ? 'yes' : 'no') : 'dry') + '">' + sideCn + ' 开→平</span><div class="detail">' + open + ' → ' + close + ' · ' + pnlTxt + syn + '</div>';
        }
        return '<span class="badge dry">' + sideCn + ' 持仓中</span><div class="detail">开仓: ' + open + syn + '</div>';
      }
      ...
      function row(r, index) {
        ...
        const relation = orderRelationHtml(findOrderForRecord(r));
        return '<tr class="' + cls + '" data-index="' + index + '"' + tsAttr + '><td class="ts" data-label="时间">' + ts + '</td><td data-label="信号">' + signal + '</td><td data-label="计划">' + plan + '</td><td data-label="新闻">' + news + '</td><td data-label="执行">' + exec + '</td><td data-label="关联订单">' + relation + '</td><td class="reason" data-label="原因">' + reason + '</td></tr>';
      }
      function render() {
