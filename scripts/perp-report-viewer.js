...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #chart-wrap { height: 420px; margin-bottom: 16px; border: 1px solid var(--border); border-radius: 8px; overflow: hidden; position: relative; }
...
<body>
  ...
  <div class="chart-header">
    ...
    <span class="meta" id="meta-symbol">· 鼠标悬停决策点显示详情，点击可放大该段</span>
  </div>
  ...
  <script>
    ...
    function init(RECORDS, OHLCV_BY_TF, MARKERS_BY_TF, symbol) {
      document.getElementById('meta-symbol').textContent = symbol + ' · 鼠标悬停决策点显示详情，点击可放大该段';
      ...
      function setTf(tf) {
        ...
        candleSeries.setMarkers((MARKERS_BY_TF[tf] || []).map(m => ({ ...m, time: m.time })));
      ...
      function showPopoverPinned(record) {
        ...
        popover.classList.add('visible');
        applyDecisionPriceLines(record);
        popover.style.left = (rect.width - 340) + 'px'; popover.style.top = '8px';
        popoverPinned = true;
      }
      ...
      function getRecordAtCoord(x) {
        ...
        const matched = RECORDS.filter(r => { if (!r.ts || r.stage) return false; const ts = new Date(r.ts).getTime() / 1000; return ts >= barStart && ts < barEnd; });
      ...
      chart.subscribeCrosshairMove(function(param) {
        ...
        const record = getRecordAtCoord(param.point.x);
      ...
      function row(r, index) {
        ...
        return '<tr class="' + cls + '" data-index="' + index + '"' + tsAttr + '><td class="ts">' + ts + '</td><td>' + signal + '</td><td>' + plan + '</td><td>' + news + '</td><td>' + exec + '</td><td class="reason">' + reason + '</td></tr>';
      }
      function render() {
