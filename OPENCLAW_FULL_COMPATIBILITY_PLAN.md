...
以下 3 项来自当前真实使用反馈，定义为 **阻断级 P0**，优先级高于其它兼容项：

1. **“不能做事”体验（新闻/修代码等）**  
   - 症状：新闻查询经常返回“无法获取实时信息”；工具执行像脚本兜底。  
   - 验收：新闻类请求默认走实时抓取链路；失败时提供可解释降级，不再空泛拒答。

2. **多轮上下文记忆弱**  
   - 症状：几轮后忘记前文约束，无法持续跟踪用户意图。  
   - 验收：同会话内持续引用前文事实/偏好；上下文压缩后仍可保留关键约束。

3. **TG 会话锁冲突**  
   - 症状：`session file locked ... .jsonl.lock` 导致回执失败。  
   - 验收：同 session 串行执行 + 锁冲突重试退避；异常率显著下降，避免直接失败回执。
